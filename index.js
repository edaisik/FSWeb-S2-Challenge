...
function cumlelereDonustur(dizi, ayrac = ",") {
  const cumleler = dizi.map((cumle) => cumle.join(ayrac));
  return cumleler;
}
...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur) {
  let dizi = [];
  dizi = cumlelereDonustur(cumleler, " ");
...
console.log(
  emojileriDonustur(
    "Merhaba :) Workintech'de <3  ikinci :O haftayı :d geride :p bırakmış :( bulunmaktayız :D",
    emojiler
  )
);
